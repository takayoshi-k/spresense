...
void nximage_image(char *image, int position, int width, int height)
{
  FAR struct nxgl_rect_s dest;
  ...
  dest.pt1.x = (position % g_nximage.xres);
  dest.pt1.y = (position / g_nximage.xres);;
  ...
  for (cnt=0;
    ...
    {
      src[0] = image;
      ret = nx_bitmap((NXWINDOW)g_nximage.hbkgd, &dest, src, &dest.pt1,
                      width * sizeof(nxgl_mxpixel_t));
      if (ret < 0)
        {
          printf("nximage_image: nx_bitmapwindow failed: %d\n", errno);
        }
